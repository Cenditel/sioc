 ...
 * Tabla:	tabla8
 * Tipo:	Básica
 * Tema:	Material predominante del piso
 * <Descripcion: tabla básica para registrar los tipos de materiales
 * de los que pueden estar hechos los pisos>
 */
class tabla8 extends P4A_Simple_Edit_Mask
{
    public function __construct()
    {
        ...
        $this->setTitle("Material Utilizado en el Piso");
        ...
        $this->display("menu", $p4a->menu)
            ->setFocus($this->fields->tabla8_campo2);
    }
    ...
    private function camposObligatorios()
    {
         ...
         */
        $this->setRequiredField("tabla8_campo2");
    ...
    private function propiedadesCampos()
    {
        ...
        $this->fields->tabla8_campo2
        	->setLabel("Descripción")
        	->setWidth(200)
        	->setProperty('maxlength',"50")
        	->addValidator(new P4A_Validate_Alpha(true),true);
        $this->fields->tabla8_campo3->setLabel("Visible");

        $this->fields->tabla8_campo2->label->setWidth(120);
        $this->fields->tabla8_campo3->label->setWidth(120);
 ...
 */
		try
		{
            ...
            $tabla8_campo1 = $this->fields->tabla8_campo1->getSQLValue();
            $tabla8_campo2 = $this->fields->tabla8_campo2->getSQLNewValue();
            $tabla8_campo3 = $this->fields->tabla8_campo3->getSQLNewValue();

            $existe = "SELECT count(*) FROM cc1.tabla8 WHERE tabla8_campo3 = true AND activo = '$tabla8_campo3' AND tabla8_campo2 ILIKE('$tabla8_campo2'); ";
            $this->user_data = $p4a_db->fetchRow($existe);
            if ($this->user_data['count'] >= 1) {
                $this->warning("El registro ya existe");
                $this->setFocus($this->fields->tabla8_campo2);
                return;
            }

            $query = "INSERT INTO cc1.tabla8 (tabla8_campo2, tabla8_campo3) VALUES (INITCAP('$tabla8_campo2'), '$tabla8_campo3')";
            $p4a_db->query($query);
            if ($tabla8_campo1)
                P4A_DB::singleton()->query("UPDATE cc1.tabla8 SET activo = 'false' WHERE tabla8_campo1 = $tabla8_campo1");
