 ...
 * Tabla:	tabla4
 * Tipo:	Básica
 * Tema:	Parentescos familiares
 * <Descripcion: tabla básica para registrar los Prentescos que tiene
 * las personas, con el jefe del hogar.
 * Se debe incluir el Jefe del Hogar como un parentesco para garantizar
 * su existencia>
 */
class tabla4 extends P4A_Simple_Edit_Mask
{
...
//         $this->toolbar->buttons->delete->disable();
        ...
        $this->display("menu", $p4a->menu)
            ->setFocus($this->fields->tabla4_campo2);
    }
    ...
    private function camposObligatorios()
    {
         ...
         */
        $this->setRequiredField("tabla4_campo2");
...
//         $this->toolbar->buttons->delete->disable();
         ...
         *
         * la columna <tabla4_campo1> la ocultamos
         * la columna <fregistro> la ocultamos
         * la columna <activo> la ocultamos
         */
        $this->table->cols->tabla4_campo1->setVisible(false);
        $this->table->cols->fregistro->setVisible(false);
        $this->table->cols->activo->setVisible(false);
        $this->table->cols->tabla4_campo2->setLabel("Nombre");
        $this->table->cols->tabla4_campo3
        	->setLabel("Visible")->setWidth(50);

        /**
         * Propiedades de los Campos del Formulario
         *
         * el campo <tabla4_campo1> lo desactivamos
         * el campo <fregistro> lo desactivamos
         * el campo <activo> lo desactivamos
         * al campo <tabla4_campo2> lo validamos contra ...
         */
        $this->fields->fregistro
        	->setVisible(false);
        $this->fields->activo->setVisible(false);

        $this->fields->tabla4_campo1
        	->disable()->setVisible(false);
        $this->fields->tabla4_campo2
        	->setLabel("Descripción")
        	->setWidth(200)
        	->setProperty('maxlength',"50")
        	->addValidator(new P4A_Validate_Alpha(true),true);
        $this->fields->tabla4_campo3->setLabel("Visible");

        $this->fields->tabla4_campo2->label->setWidth(120);
        $this->fields->tabla4_campo3->label->setWidth(120);
    ...
    public function saveRow()
	{
		try
		{
            ...
            $tabla4_campo1 = $this->fields->tabla4_campo1->getSQLValue();
            $tabla4_campo2 = $this->fields->tabla4_campo2->getSQLNewValue();
            $tabla4_campo3 = $this->fields->tabla4_campo3->getSQLNewValue();

            $existe = "SELECT count(*) FROM cc1.tabla4 WHERE tabla4_campo3 = true AND activo = '$tabla4_campo3' AND tabla4_campo2 ILIKE('$tabla4_campo2'); ";
            $this->user_data = $p4a_db->fetchRow($existe);
            if ($this->user_data['count'] >= 1) {
                $this->warning("El registro ya existe");
                $this->setFocus($this->fields->tabla4_campo2);
                return;
            }

            $query = "INSERT INTO cc1.tabla4 (tabla4_campo2, tabla4_campo3) VALUES (INITCAP('$tabla4_campo2'), '$tabla4_campo3')";
            $p4a_db->query($query);
            if ($tabla4_campo1)
                P4A_DB::singleton()->query("UPDATE cc1.tabla4 SET activo = 'false' WHERE tabla4_campo1 = $tabla4_campo1");
